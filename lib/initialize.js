...
module.exports = function (sails) {
  return async function (cb) {
    ...
    peer.on('status::joined', async () => {
      ...
      const response = await emitAll(peer, 'info', {
        id: peer.self.id,
        data: nodeData[peer.self.id]
      });
    ...
    sails.after('hook:moduleloader:loaded', function () {
      ...
      setupListeners(peer, nodeData, lastUpdate, endpoints);
    });
...
function setupListeners(peer, nodeData, lastUpdate, endpoints) {
...
async function emitAboutModels(thisPeer, lastUpdate, emitFunc, peer, models) {
  models = models || sails.models;
