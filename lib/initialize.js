...
module.exports = function (sails) {
  return async function (cb) {
    ...
    const lastUpdate = conf.lastUpdate || new Date().getTime() - 24 * 60 * 60 * 1000;
...
function setupModelsListeners(peer) {
  ...
  for (let modelName of Object.keys(models)) {
    peer.handle[modelName + '.afterCreate'] = async function (values, cb) {
      ...
      const record = await models[modelName].findOne(values.id);
      if (!record)
    ...
    peer.handle[modelName + '.afterUpdate'] = function (values, cb) {
      ...
      models[modelName].update(values.id, values).exec(err => {
        if (err) sails.log.error(err);
        cb();
      });
    };

    peer.handle[modelName + '.afterDestroy'] = function (id, cb) {
      // sails.log.verbose('remoteAfterDestroy', id);
      models[modelName].destroy(id).exec(err => {
        if (err) sails.log.error(err);
        cb();
      });
    };
  ...
  for (let modelName of Object.keys(models)) {
    ...
    model.afterCreate = (previousAfterCreate =>
      async function (values, cb) {
        // sails.log.verbose('afterCreateFunc');
    ...
    model.afterUpdate = (previousAfterUpdate =>
      async function (values, cb) {
        // sails.log.verbose('afterUpdateFunc');
        if (values.peerIdEmitFrom === peer.self.id)
          await emitAll(peer, modelName + '.afterUpdate', values);

        if (typeof previousAfterUpdate === 'function') {
          previousAfterUpdate(values, cb);
        } else {
          cb();
        }
      })(model.afterUpdate);

    model.afterDestroy = (previousAfterDestroy =>
      async function (values, cb) {
        // sails.log.verbose('afterDestroyFunc');
        if (values.peerIdEmitFrom === peer.self.id)
          await emitAll(peer, modelName + '.afterDestroy', values.id);

        if (typeof previousAfterDestroy === 'function') {
          previousAfterDestroy(values, cb);
        } else {
          cb();
        }
      })(model.afterDestroy);
