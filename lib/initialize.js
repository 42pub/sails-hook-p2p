...
function emitAll(thisMesh, name, ...args) {
  return new Promise(() => {
    if (typeof args[args.length - 1] === 'function') {
      ...
      sails.log.silent('counter set', counter);
      let response = [];
      args.push((err, res) => {
        counter--;
        sails.log.silent('counter', counter, err, res);
        if (err) {
  ...
  for (let modelName of Object.keys(models)) {
    if (!modelsGrab.length || modelsGrab.includes(modelName)) {
      ...
      mesh.onRemote(modelName + '.afterUpdate', async function (values) {
        ...
        await models[modelName].update(values.p2pid, values);
      });
    ...
    if (!modelsPublic.length || modelsPublic.includes(modelName)) {
      ...
      patch(model, 'afterCreate', async function (values) {
        if (values.peerIdEmitFrom === mesh.self.id) {
          values = await model.findOnePopulate(values);
  ...
  for (let modelName in models) {
    if (models.hasOwnProperty(modelName)) {
      if (!modelsPublic.length || modelsPublic.includes(modelName)) {
        ...
        sails.log.silent('created', createdModels);
        ...
        sails.log.silent('updated', updatedModels);
