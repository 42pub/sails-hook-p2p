...
module.exports = function (sails) {
  return async function (cb) {
    ...
    const p2pOpts = {
      host: 'localhost',
      port: parseInt(sails.config.port) + 1,
    ...
    if (!modelsGrab.length || modelsGrab.includes(modelName)) {
      ...
      mesh.onRemote(modelName + '.afterUpdate', async function (values) {
        ...
        if (hashFromModel(myModel) !== hashFromModel(values)) {
          await models[modelName].update({p2pid: values.p2pid}, values);
        }
      });
...
function patchModelAttributes(model, mesh) {
  model.attributes.peerIdEmitFrom = {type: 'string', defaultsTo: mesh.self.id};
  model.attributes.p2pId = {type: 'string', unique: true, required: true};
}
    ...
    if (!modelsPublic.length || modelsPublic.includes(modelName)) {
      ...
      patch(model, 'afterCreate', async function (values) {
        if (values.peerIdEmitFrom === mesh.self.id) {
          ...
          values = await sails.models[modelName].findOnePopulate(values[id]);
...
function getIdField(model) {
  return Object.entries(model.attributes).find(([, type]) => type.primaryKey)[0];
}

function decorateModels() {
  function populateModelFields(actionName, newActionName, model) {
    model[newActionName] = function () {
      return model[actionName].apply(this, arguments).populate(getAssociations(model));
    }
  }
  ...
  for (let modelName of Object.keys(models)) {
    if (!modelsPublic.length || modelsPublic.includes(modelName)) {
      ...
      populateModelFields('findOne', 'findOnePopulate', model);
    }
  }
}
